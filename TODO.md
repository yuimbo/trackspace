...
- [ ] Add cache invalidation tests that verify semantic/layout cache keys change when source versions, feature mask, or blend parameters change.
...
- [ ] Add timing instrumentation logs around `compute_projection` stages (gather, semantic weighting, projection backend) for easier profiling and regression detection.
...
- [ ] Add DB-level integrity checks at startup (`PRAGMA integrity_check`, optional) with warning logs and recovery guidance when corruption is detected.
